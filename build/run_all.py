...
def main():
    ...
    for curr_comp in target_components:
        ...
        logger.info("Trying to Run Component:", component_name)
        ret_val = __run_component(curr_comp, component_times)
        if ret_val:
            logger.info("Component:", component_name, " passed successfully.")
        else:
            logger.error("Component:", component_name, " failed. Exiting.")

    logger.info("Component Runtime information:")
    for curr_comp in component_times:
        logger.info(curr_comp + ":" + str(component_times[curr_comp]) + " seconds.")
...
def __run_component(component_obj, component_times):
    ...
    if setup_msg is None:
        logger.info("Setup for component:", component_obj.get_name(), " complete")
        st_time = time.time()
        ret_val = component_obj.perform()
        total_time = time.time() - st_time
        component_times[component_obj.get_name()] = total_time
        if ret_val:
            logger.info("Component:", component_obj.get_name(), " ran successfully.")
            return True
        else:
            logger.error("Component:", component_obj.get_name(), " failed.")
            # Ignore if the component is not critical.
            if not component_obj.is_critical():
                return True
    else:
        logger.error("Setup failed for component:", component_obj.get_name(), ", with Error:", setup_msg)
    return False
