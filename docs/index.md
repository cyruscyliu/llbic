---
title: All You Need Is llbic
---

# All You Need Is llbic
...
If you want LLVM bitcode from a Linux kernel build, the hard part is not
running Clang. The hard part is deciding what "the bitcode for the kernel"
should actually mean.

That question shaped `llbic`.

Early versions of this project tried to reconstruct a whole-program kernel
bitcode artifact by replaying the build in LLVM space. That sounded attractive,
but it turned out to be brittle, incomplete, and difficult to keep correct
across kernel versions, architectures, and toolchains. Modern `llbic` takes a
more pragmatic view: run a reproducible kernel build, then collect the LLVM
bitcode that the chosen strategy naturally produces.

This page explains that shift: from a fragile whole-program reconstruction idea
to a reproducible artifact collection workflow that matches how the kernel is
actually built.
...
## Four Ways to Think About Kernel Bitcode
...
The rest of the article is really about why `llbic` moved away from model (2)
and settled on models (3) and (4).
...
## Why This Direction Holds Up Better
